<h3 align="center">
  ...
  <p align="center"><img src="https://img.shields.io/badge/WLCM%20TO -RANA MEDIA DOWNLOADER-green?colorA=%23ff0000&colorB=%23017e40&style=flat-square">  
  
</h3>
 
[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Neuton&size=25&color=30FF40&background=000000¢er=true&vCenter=true&width=360&height=60&lines=Hello+World%2C+I'm+Mr-RANA+Here+🤙;𝙸𝚃'𝚜+𝙽𝙾𝚃+𝙰+𝙹𝚄𝚂𝚃+𝙽𝙰𝙼𝙴+𝙱𝚁𝙾+🥱;𝙸𝚃'𝚜+𝙰+𝙱𝚁𝙰𝙽𝙳+🔥;Respect+Mr.RANA+🥀;Thanks+My+All+Friend+🤙+🥰)](https://git.io/typing-svg)
 
 
 
## Instalation :
```bash
> npm i rana-videos-downloader
```
## 🚀 Project Update Log  
...
## Example (fb & insta)
```js
const { ndown } = require("rana-videos-downloader")
...
## Example (YouTube)
```js
const { ytdown } = require("rana-videos-downloader")
...
## Example Fbdown2
```js
const { fbdown2 } = require("rana-videos-downloader");
...
## Usage GDLink 
```js
const {GDLink} = require("rana-videos-downloader");
...
## Usage Pintarest 
```js
const {pintarest} = require("rana-videos-downloader");
...
## Usage CapCut 
```js
const { capcut } = require("rana-videos-downloader");
...
## Usage Threads
```js
const { threads } = require("rana-videos-downloader");
...
```js
(async () => {
  const { spotifySearch } = require("rana-videos-downloader");
...
```js
(async () => {
  const { spotifyDl } = require("rana-videos-downloader");
...
```js
(async () => {
  const { soundcloudSearch } = require("rana-videos-downloader");
...
```js
(async () => {
  const { soundcloud } = require("rana-videos-downloader");
...
```
```js
const {alldown} = require("rana-videos-downloader");
